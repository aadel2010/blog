...
const ANIMATION_CONFIG = {
    hero: {
        diagonalLines: { count: 0, opacity: 0.4, scaleX: 4.0 }
    }
};
...
function createAndAnimateDiagonalLines(container, options = {}) {
    const config = { count: 0, background: 'linear-gradient(45deg, transparent, var(--color-primary-light), transparent)', height: 3, width: 1200, opacity: 0.4, scaleX: 4.0, duration: 4, rotationDuration: 8, ...options };
...
function createAndAnimateSplineCurves(container, options = {}) {
    const config = { count: 0, size: 180, border: '3px solid var(--color-primary)', opacity: 0.6, scale: 1.2, duration: 6, rotationDuration: 10, ...options };
